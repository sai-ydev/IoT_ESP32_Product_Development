...
class MicroPythonI2cTransceiver(object):
    ...
    def description(self):
        ...
        :rtype: str
        """
        return "MicroPython I2C Transceiver"
    ...
    def transceive(self, slave_address, tx_data, rx_length, read_delay,
        ...
        if tx_data is not None:
            try:
                self._device.writeto(slave_address, bytes(tx_data))
            except OSError:
                status = self.STATUS_UNSPECIFIED_ERROR
                log.exception("I2C TX Failure")
        
        if read_delay:
            time.sleep(read_delay)
        
        if rx_length and status == self.STATUS_OK:
            try:
                rx_data = self._device.readfrom(slave_address, rx_length)
            except OSError:
                status = self.STATUS_UNSPECIFIED_ERROR
                log.exception("I2C RX Failure")
